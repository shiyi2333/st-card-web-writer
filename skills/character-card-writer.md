...
- 开场白指导：开场白是一条完整 first message，默认 300-500 个汉字，至少三段自然段，段落之间空行。不要写成一团，不要拆成多个对话片段、项目符号、编号或 alternate starts。
- 开场白指导：直接进入场景，写地点、姿态或动作、关系张力，以及让玩家自然回应的钩子。用户用“你”代指，角色部分用第三人称、角色姓名或自然代称来写；不要在自然叙事里反复写“{{user}}”。
- 开场白指导：成人女性角色、肉感角色、安产型体态、衣物受力、露骨成人文风相关需求，必须调用或遵循 `openclaw-erotic-style` 里的原文肉感文风；不要改写、总结、淡化这份文风规范。
...
- 需要成人肉感文风时，优先调用或遵循 `st-card-style-guide` 与 `openclaw-erotic-style`；导出时默认世界书会带入 OpenClaw 色情文风原文，不要把这些规则写成 Markdown 列表塞进角色正文。
